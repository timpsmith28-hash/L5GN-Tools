...
- [ ] [W] **A1.** The left nav lists all 9 projects with their **authored** counts.
...
- [ ] [W] **A4.** **The content matches the file.** Open the same file in an editor
      ...
      *the file*.
- [ ] [W] **A5.** Edit that file in your editor, save, and click the document again
...
- [ ] [W] **A6.** No generated document is offered anywhere. `L5GN_Armory_v4` shows
...
- [ ] [W] **B1.** Search a term you know is in one of the MCF or L5GN knowledge
      documents but cannot remember the location of. The status line reads
      `N result(s) · engine: fts5 · scope: whole estate`.
- [ ] [H] **B2.** **Does it surface the right document?** Judge this honestly. If
      ...
      is worth nothing.
- [ ] [W] **B3.** The snippet shows the match highlighted in context, and the hit
      names the project, doc_type and path.
- [ ] [W] **B4.** A hit from a `knowledge` document has a green left edge and sorts
      ahead of equally-relevant ordinary documents.
- [ ] [W] **B5.** Click a hit. It jumps to the Documents tab, selects that project,
      and opens that document at the right place.
- [ ] [W] **B6.** Set the scope dropdown to one project and repeat. Results are
      restricted to it.
- [ ] [W] **B7.** Search something certainly absent (`zzqqxx`). "Nothing matched"
      reads as a clean empty state, not an error.
- [ ] [W] **B8.** Search a deliberately broken FTS5 expression — a bare `"` — and
...
- [ ] [G] **C2.** Try a real path as the id:
...
- [ ] [G] **C3.** Try a plausible-looking forged digest: `?doc_id=0000000000000000`.
      Expect **404 unknown_document**.
- [ ] [G] **C4.** Confirm there is **no route anywhere** that accepts a path. Open
      `http://127.0.0.1:8002/api/docs` and read the estate route signatures:
      every one takes `project`, `doc_id` or `q`. None takes a path.
- [ ] [W] **C5.** Header warnings: if the build stamp shows a "warning(s)" count,
...
- [ ] [G] **D1.** Point the machine at a vault path that does not exist (rename the
...
- [ ] [W] **D2.** The **Documents**, **Search** and **Time** tabs all work normally.
- [ ] [W] **D3.** The **Review queue** tab says the queue is unavailable on this
...
- [ ] [G] **D4.** `curl http://127.0.0.1:8002/api/health` shows `vault.available:
      false` with a reason, and `estate.available: true`. Both halves reported
      separately.
- [ ] [W] **D5.** The mirror case: restore the vault and temporarily rename
...
- [ ] [G] **D6.** With **both** absent, `run.py review` refuses with exit code 2 and
...
- [ ] [W] **E1.** The build stamp is the first thing on the page and states
      `generated_at`, the age, and the toolkit commit.
- [ ] [W] **E2.** `toolkit_dirty` at build time shows an amber "toolkit dirty at
      build time" flag. (It is currently **true** on `ac7710d`, so expect to see
      it — that is the flag working, not a fault.)
- [ ] [W] **E3.** **Make a build read as stale.** Point the deck at an older
...
- [ ] [G] **F1.** Before starting the server, capture the state of `data/`:
...
- [ ] **F2.** Browse several documents, run several searches, open the Time tab.
      *(unmarked — a setup action for F3, not an independent check; see
      COWORK_REPORT_ui_witness.md Task 1.)*
- [ ] [G] **F3.** Stop the server. Re-run the same command. **The file count is
      unchanged** and `git status` shows no new file under `data/`.
- [ ] [G] **F4.** Search `data/` for any index or cache artefact:
      `Get-ChildItem data -Recurse -Include *.db,*.sqlite,*.idx,*.cache`.
      Expect nothing new.
- [ ] [G] **F5.** `curl http://127.0.0.1:8002/api/estate/search/status` reports
      `"persisted": false`.
...
- [ ] [W] **G1.** The estate timeline draws every project with history on one shared
      axis, with the date range and total span in the heading.
- [ ] [W] **G2.** **The lineage is visible.** Armory → Armory_v2 → Armory_v4 sit in
      ...
      *(Classed witness, not human: the expected order is a computable fact —
      git chronology — not an opinion; see COWORK_REPORT_ui_witness.md Task 1.)*
- [ ] [W] **G3.** The "Per project" table gives first commit, last commit, days,
      commit count and contributors. **Contributors read `L5GN`**, not
      `timpsmith28-hash` — the alias fold is working.
- [ ] [W] **G4.** **`L5GN-Archive` appears under "No history — stated, not guessed"**
      ...
      appear here.
- [ ] [W] **G5.** Hover a timeline bar. The tooltip gives the real dates, span and
      commit count. A project whose whole history is one afternoon still shows a
      visible marker but a truthful number beside it.
- [ ] [W] **G6.** "What changed since the last build" **names both builds** by file,
      ...
      unreadable.
- [ ] [W] **G7.** The delta's claims are checkable: pick one project it says gained
      commits and confirm against `git -C <project> log`. *(Classed witness for
      the same reason as G2: checkable against git log, not a judgement call.)*
...
- [ ] [W] **H1.** On a machine **with** a vault: the Review queue tab loads its
      project nav and batches exactly as before.
- [ ] [W] **H2.** Accepting a batch, "Not this project", and the "→ other candidate"
      button all still work.
- [ ] [W] **H3.** Switching between tabs does not lose the queue's selected project
      or its loaded batch.
